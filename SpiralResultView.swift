...
struct SpiralResultView: View {
    ...
    func analyzeImage(){
        ...
        do{
            ...
            let model=try parkinson_spiral(configuration: config)
            let pinput=parkinson_spiralInput(input_1: resultImage.mlMultiArrayGrayScale(scale:1))
    ...
    struct SpiralResultView_Previews: PreviewProvider {
        static var previews: some View {
            SpiralResultView(path: .constant(NavigationPath()), canvasView: PKCanvasView())
        }
    }
...
extension UIImage {
    func redrawing(overColor color: UIColor, opaque: Bool = true) -> UIImage {
    ...
    func mlMultiArrayGrayScale(scale preprocessScale:Double=255,bias preprocessBias:Double=0) -> MLMultiArray {
        ...
        let size = self.size
        //print("image pixel",imagePixel)
        let imagePointer : UnsafePointer<Double> = UnsafePointer(imagePixel)
        let mlArray = try! MLMultiArray(shape: [1,  NSNumber(value: Float(size.width)), NSNumber(value: Float(size.height)),1], dataType: MLMultiArrayDataType.double)
    ...
    func getPixelGrayScale(scale preprocessScale:Double=255, bias preprocessBias:Double=0) -> [Double]
    {
        ...
        for j in 0..<height {
            for i in 0..<width {
                ...
                var v = Double(pixelData[pixelInfo])
                /*if v<Double(0.5){
                 v=Double(255.0)
                 }*/
